...
class BRCDataset(object):
    ...
    def _load_dataset(self, data_path, train=False):
        ...
        return data_set
    #_load_dataset01()用于对DESCRIPTION、ENTITY、YES_NO三种不同类型问题进行评估
    def _load_dataset01(self, data_path, train=False):
        """
        Loads the dataset
        Args:
            data_path: the data file to load
        ...
        for sidx, sample in enumerate(batch_data['raw_data']):
            for pidx in range(max_passage_num):
                if pidx < len(sample['passages']):
                    ...
                    batch_data['passage_length'].append(min(len(passage_token_ids), self.max_p_len))#超过500是500，不超过就是真实长度
                else:
