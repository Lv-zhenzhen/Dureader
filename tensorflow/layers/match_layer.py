...
class MatchLSTMLayer(object):
    ...
    # sep_p_encodes:[2 * max_p_len, hidden_size], #sep_q_encodes:[2 * max_q_len, hidden_size]
...
class AttentionFlowMatchLayer(object):
    ...
    def match(self, passage_encodes, question_encodes, p_length, q_length):
        ...
        with tf.variable_scope('bidaf'):
            sim_matrix = tf.matmul(passage_encodes, question_encodes, transpose_b=True) # [2 * p_len, 2 * q_len]
            context2question_attn = tf.matmul(tf.nn.softmax(sim_matrix, -1), question_encodes)#[2 * p_len ,hidden_size]
            ...
            ###########################################################################
            # 加入自注意力机制
            # sim_matrix_1 = tf.matmul(passage_encodes, passage_encodes, transpose_b=True)
            # context2context_attn = tf.matmul(tf.nn.softmax(sim_matrix_1, -1), passage_encodes)
            #############################################################################
            concat_outputs = tf.concat([passage_encodes, context2question_attn,#context2context_attn,
